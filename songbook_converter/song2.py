...
from song_types import ILine, ITextRun, ISong, ICategory, IEditorInfo, ISongKey, IKey, INote, IVerse, IChord, \
    IComplexChord, IChordSeries, IChords
...
def __get_paragraph_from_runs(runs: list[Run], indent: int) -> list[ILine]:
    ...
    for run in runs:
        ...
        if current_type == TextType.TEXT:
            ...
            __parse_chords_or_repetitions(lines[-1], run)

    if indent is not None:
        indents = int((indent / 265430.0) + 0.9)
        for line in lines:
            if len(line.text) > 0 and len(
                    line.text[0].text) > 0:  # TODO czy pierwszy warunek jest potrzebny (czasem się psuje)
                line.text[0].text = "\t" * indents + line.text[0].text

    return lines


def __parse_chords_or_repetitions(line: ILine, run: Run):
    text = run.text.strip()
    if len(text) == 0:
        return
    if not line.chords:
        line.chords = IChords([])
    if text.startswith("|"):
        if len(line.chords.chords) == 0:  # repetition
            line.repetition = True
            after = text[text.index("|") + 1:]
            if after.startswith("x"):
                if after[1].isnumeric():
                    line.repetitionEnd = int(after[1])
                elif after[1] == "∞":
                    line.repetitionEnd = -1
        elif len(line.chords.alternatives) == 0:  # alternatives
            pass  # TODO
        else:
            raise Exception('Znak repetycji w nieprawidlowym miejscu')

    else:
        if run.font.subscript:
            pass  # TODO base
        elif run.font.superscript:
            pass  # TODO additionals
        elif not run.italic:
            # TODO optionals
            ...
            if len(text) == 0:
                return
            chords = text.split(" ")
            if not line.chords.chords:
                line.chords.chords = [IChordSeries([])]
            line.chords.chords[0].chords.extend([__parse_complex_chord(chord) for chord in chords])


def __parse_complex_chord(text: str) -> IComplexChord:
    chords = text.split("/")
    chord = IComplexChord(__parse_simple_chord(chords[0]))
    if len(chords) == 2:
        chord.alternative = __parse_simple_chord(chords[1])
    elif len(chords) > 2:
        raise Exception(text + ' is not proper complex chord')
    return chord


def __parse_simple_chord(text: str) -> IChord:
    original_text = text
    note: str
    ...
    if text[0].upper() == 'B':
        ...
        if not text[0].isupper():
            minor = True
        if note not in ['C', 'D', 'E', 'F', 'G', 'A', 'H']:
            raise Exception(note + 'Nieprawidlowy akord')
        text = text[1:]
    ...
    if len(text) > 0:
        raise Exception(original_text + ' is not proper chord')

    return IChord(INote(note, accidental), minor, modification)
